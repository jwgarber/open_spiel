...
//
// Parameters:
//   "base_size"         int     size of the base of the board (default = 7)
...
inline constexpr int kDefaultBaseSize = 7;
...
inline constexpr int BoardSize(int base_size) {
...
struct Move {
  ...
  inline constexpr Move(int cell_) : cell(cell_) {}

  std::string ToString() const;

  bool operator==(const Move& b) const { return cell == b.cell; }
  bool operator!=(const Move& b) const { return cell != b.cell; }

  // Flags for which edge this move is part of.
  BoardEdge Edge(int board_size) const;
};

// List of neighbors of a cell: [cell]
using NeighborList = std::vector<std::vector<Move>>;
...
class GeodesicYState : public State {
  ...
  struct Cell {
    ...
    Cell() {}
    Cell(GeodesicYPlayer player_, int parent_, BoardEdge edge_)
